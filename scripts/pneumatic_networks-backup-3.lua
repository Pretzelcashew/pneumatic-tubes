...
local function redistribute_harvested_capsules(harvested_capsules, fallback_pos)
    ...
    for _, entry in ipairs(harvested_capsules) do
        local cap_key = entry.key
        ...
        local surface = (fallback_pos and fallback_pos.surface) or (game and game.surfaces["nauvis"])

        local nearest_net_id = nil
        local min_dist = math.huge

        if pos then
            for net_id, net_struct in pairs(storage.pneumatic_networks or {}) do
                for u_num, member in pairs(net_struct) do
                    if u_num ~= "capsules" and u_num ~= "length" and member and member.valid then
                        local dx = member.position.x - pos.x
                        local dy = member.position.y - pos.y
        ...
        if nearest_net_id and min_dist <= 6.25 then
            local dest_net = storage.pneumatic_networks[nearest_net_id]
            dest_net.capsules = dest_net.capsules or {}
            dest_net.capsules[cap_key] = capsule
            remapped = true
        end
        ...
        if not remapped then
            ...
            if surface and surface.valid then
                surface.spill_item_stack{ position = spill_pos, stack = { name = "item-capsule", count = 1 } }
                if capsule.payload_name and capsule.cargo then
                    local q_name = "normal"
                    if capsule.payload_quality then
                        if type(capsule.payload_quality) == "string" then
                            q_name = capsule.payload_quality
                        elseif type(capsule.payload_quality) == "table" or type(capsule.payload_quality) == "userdata" then
                            q_name = capsule.payload_quality.name or "normal"
                        end
                    end
                    surface.spill_item_stack{
                        position = spill_pos,
                        stack = { name = capsule.payload_name, count = capsule.cargo, quality = q_name }
                    }
                end
            end
        end
...
local function on_entity_removed(e)
    ...
    -- Harvest target networks across all physical clusters affected by the removal
    ...
    for _, island in ipairs(merged_islands) do
        for u_num, _ in pairs(island) do
            local nets = storage.entity_to_network and storage.entity_to_network[u_num]
            if nets then
                for net_id, _ in pairs(nets) do
                    target_net_ids[net_id] = true
                end
    ...
    if storage.pneumatic_networks then
        for net_id, _ in pairs(target_net_ids) do
            ...
            if net_struct and net_struct.capsules then
                for cap_key, capsule in pairs(net_struct.capsules) do
                    table.insert(harvested_capsules, { key = cap_key, capsule = capsule })
                end
            end
        end
    end
